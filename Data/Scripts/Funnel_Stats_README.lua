--[[
    ...
    1) This module tracks up to 24 different player actions, allowing creators to see how Players 
    interact with their game.
    
    2) Currently, a Player's first session time (Up to 9,999 seconds) will be tracked and stored.
    
    3) Each Player is put into either group 1 (Group A) or 2 (Group B) based on Player.id, this allows
    creators to split test, various functionality into their games.
    ...
    2. Create a new Net Reference - Global Leaderboard of up to 500 entires and
       drag it into "FunnelStats" as a custom property found in FunneStatTracker
    ...
    a max of 24 steps can be tracked, if you for whatever reason do not wish to track all 24 steps, simply change
    their names to a place holder, rather than deleting them.

    7. To track a step as complete, fire _G.Funnel.SetPlayerStepComplete(Player, stepIndex) after the player 
    completed the approriate interaction. _G.Funnel.SetPlayerStepComplete(Player, stepIndex) can be used from both
    ...
    example the tutorial isn't shown to groupId 2.
